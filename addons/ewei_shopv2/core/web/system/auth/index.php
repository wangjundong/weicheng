...
class Index_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			$resp = ihttp_request('http://999.v-999.com/mod.php?a=mod', array('ip' => $ip, 'modname' => $modname, 'code' => $_GPC['code'], 'domain' => $domain));
			...
			if ($result['status'] == '1') {
				...
				if(!isset($result['status'])) show_json(0,'已授权，请勿重复点击验证授权！');
