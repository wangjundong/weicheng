...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		$resp = ihttp_request('http://999.v-999.com/mod.php?a=mod&b=check', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request('http://999.v-999.com/mod.php?a=mod&b=download', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '亲测网',`url` =  'http://www.qincewang.com' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
