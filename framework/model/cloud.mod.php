...
define('CLOUD_GATEWAY_URL', 'http://api.we7.cc/gateway.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://api.we7.cc/gateway.php');
define('CLOUD_GATEAUTH_URL', 'http://api.we7.cc/gateway.php');
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系微信客服！');
	}
...
function cloud_prepare() {
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		return error('-1', "您的站点只有在微信云服务平台成功注册后，才能使用云服务的相应功能。");
	}
