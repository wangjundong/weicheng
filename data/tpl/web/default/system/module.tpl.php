...
<div id="js-system-module" ng-controller="installedCtrl" ng-cloak>
	<ul class="we7-page-tab">
		<li class="active"><a href="<?php  echo url('system/module/installed')?>">已安装公众号应用  </a></li>
		<li><a href="<?php  echo url('system/module/not_installed')?>" ng-if="isFounder == 1">未安装公众号应用<span class="color-red">  (<?php  echo $total_uninstalled;?>) </span></a></li>
		<li><a href="<?php  echo url('system/module/not_installed', array('status' => 'recycle'))?>" ng-if="isFounder == 1">已停用公众号应用</a></li>
	</ul>
		...
		<table class="table we7-table table-hover vertical-middle table-manage">
			...
			<tr ng-repeat="module in module_list">
				...
				<td class="text-left">
					...
					<span class="color-red" ng-if="module.service_expire">升级服务到期</span>
				</td>
				<td>
					<div class="link-group">
						<a ng-href="{{ './index.php?c=system&a=module&do=upgrade&module_name='+module.name}}" class="color-red del" ng-if="module.upgrade && module.from != 'cloud' && isFounder == 1">升级</a>
						<a href="<?php  echo url('system/module/module_detail')?>&name={{ module.name }}&show=upgrade" class="color-red del" ng-if="module.upgrade && module.from == 'cloud' && isFounder == 1">升级</a>
						<a href="<?php  echo url('system/module/module_detail')?>&name={{ module.name }}&account_type=<?php echo ACCOUNT_TYPE;?>&type=<?php echo ACCOUNT_TYPE_OFFCIAL_NORMAL;?>" ng-if="isFounder == 1">管理设置</a>
...
<script>
	require(['fileUploader'], function() {
		angular.module('moduleApp').value('config', {
			...
			'filterUrl' : "<?php  echo url('system/module/filter')?>"
		});
...
<div id="js-system-module-not_installed" ng-controller="notInstalledCtrl" ng-cloak>
	...
	<table class="table we7-table table-hover vertical-middle table-manage">
		...
		<tr ng-repeat="module in module_list">
			...
			<td class="text-right">
				<a href="<?php  echo url('system/module/upgrade')?>&module_name={{ module.name }}" ng-if="module.upgrade_support == true" class="btn btn-primary">安装应用模块</a>
				<a href="<?php  echo url('system/module/install')?>&module_name={{ module.name }}" ng-if="module.upgrade_support != true" class="btn btn-primary">安装应用模块</a>
			</td>
		</tr>
	</table>
	</form>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<table class="table we7-table table-hover vertical-middle table-manage">
		...
		<tr>
			...
			<td class="text-right">
				<div class="link-group"><a href="<?php  echo url('system/module/module_detail')?>name={{moduleinfo.name}}&account_type={{moduleinfo.account_type}}&type={{moduleinfo.type}}">查看</a></div>
			</td>
		</tr>
	</table>
	<?php  } ?>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud'" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr ng-if="branch.displayorder >= upgradeInfo.site_branch.displayorder">
			...
			<td class="text-right">
				...
				<a ng-href="{{ module.service_expire ? 'http://we7.cc/module-' +upgradeInfo.id + '.html' : './index.php?c=cloud&a=process&m='+upgradeInfo.name+'&is_upgrade=1' }}" ng-click="notice(service_expire)" ng-if="branch.id == upgradeInfo.site_branch.id && branch.version.version !=  moduleinfo.version" class="btn btn-primary">升级</a>
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				...
				<a href="http://we7.cc/module-{{upgradeInfo.id}}.html" ng-if="branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id)" class="color-default view-detail" target="_blank">查看分支详情</a>
			</td>
		</tr>
