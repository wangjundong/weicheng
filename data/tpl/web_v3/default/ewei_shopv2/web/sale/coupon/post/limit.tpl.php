...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12" >
        ...
        <label class="radio-inline coupon-radio">
            <input type="radio" name="limitdiscounttype" value="0" <?php  if($item['limitdiscounttype'] == 0) { ?>checked="true"<?php  } ?>  /> 无优惠使用限制
        </label>
        <label class="radio-inline coupon-radio">
            <input type="radio" name="limitdiscounttype" value="1" <?php  if($item['limitdiscounttype'] == 1) { ?>checked="true"<?php  } ?>" /> 不可与促销优惠同时使用
        </label>
        <label class="radio-inline coupon-radio">
            <input type="radio" name="limitdiscounttype" value="2" <?php  if($item['limitdiscounttype'] == 2) { ?>checked="true"<?php  } ?>  /> 不可与会员折扣同时使用
        </label>
        <label class="radio-inline coupon-radio">
            <input type="radio" name="limitdiscounttype" value="3" <?php  if($item['limitdiscounttype'] == 3) { ?>checked="true"<?php  } ?>  /> 不可与促销优惠和会员折扣同时使用
        </label>
        <span class='help-block'>优惠券是否可以与特定优惠同时使用</span>

        <?php  } else { ?> <div class='form-control-static'>
            <?php  if($item['limitdiscounttype']==0) { ?>无优惠使用限制
            <?php  } else if($item['limitdiscounttype']==1) { ?>不可与促销优惠同时使用
            <?php  } else if($item['limitdiscounttype']==2) { ?>不可与会员折扣同时使用
            <?php  } else if($item['limitdiscounttype']==3) { ?>不可与促销优惠和会员折扣同时使用
            <?php  } ?>
        </div>
        <?php  } ?>
    </div>
